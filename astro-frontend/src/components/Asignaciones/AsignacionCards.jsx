...
const handleGeneratePDF = async (codigoAsignacion) => {
  try {
    ...
    const docDefinition = {
      pageSize: "LETTER",
      pageMargins: [90, 60, 90, 40], // 2 pulgadas a los lados, superior/inferior formal
      content: [
        {
          image: logoBase64,
          width: 160,
        ...
        },
        {
          ...
          layout: "grid", // borde completo
          margin: [0, 0, 0, 12],
        },
        {
          text: `Promedio General: ${(
            (asignacion.punteo / asignacion.examen.punteo_maximo) * 100
          ).toFixed(2)}%`,
          bold: true,
          alignment: "right",
          fontSize: 11,
        ...
        ...(asignacion.punteo === 0
          ? [
              {
                text: "IMPORTANTE: El resultado de esta evaluación corresponde a un puntaje de cero (0), lo cual puede deberse a un desempeño insuficiente o a la anulación automática del examen por incumplimiento de las normas establecidas, tales como el uso de recursos no autorizados, la falta de verificación de identidad o intentos de suplantación.",
                alignment: "justify",
                fontSize: 9.5,
                color: "#D32F2F",
                margin: [0, 0, 0, 20],
  ...
  if (loading)
    return (
      <div className="text-center font-page">Cargando asignaciones...</div>
    );
      ...
      {filteredData.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredData.map((row, i) => (
            <div
              key={row.codigo_asignacion}
              className="rounded-xl overflow-hidden shadow-md border bg-white relative"
            >
              <div
                className="h-52 relative bg-cover bg-center"
                style={{
                  backgroundImage: `url(/cards/card${Math.floor(Math.random() * 5) + 1}.webp)`,
                }}
              ></div>
              <div className="p-5 space-y-2 bg-white relative z-10">
                <p>
                  <strong>Examen:</strong>{" "}
                  {row.examen?.motivo_examen?.nombre_motivo}
                </p>
                <p>
                  <strong>Tipo:</strong> {row.examen?.tipo_examen?.description}
                </p>
                <p>
                  <strong>Punteo:</strong>{" "}
                  {row.punteo === null ? "No evaluado" : row.punteo}
                </p>
