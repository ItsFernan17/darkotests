...
export async function updateAsignacion(codigo_asignacion: number, newAsignacion: any) {
  try {
    const token = getToken();
    const response = await fetch(`${backendHost}/api/v1/asignacion/${codigo_asignacion}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
export async function desactiveAsignacion(codigo_asignacion: number) {
  try {
    const token = getToken(); 
    const response = await fetch(`${backendHost}/api/v1/asignacion/${codigo_asignacion}/estado`, {
      method: 'PATCH',
      headers: {
        Authorization: `Bearer ${token}`,
      },
